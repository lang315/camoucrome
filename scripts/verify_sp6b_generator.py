...
def generate(os_name, seed, locale=None, timezone=True):
    cmd = [PY, "-m", "camoucrome.gen", "--os", os_name, "--seed", str(seed)]
    if timezone:
        cmd += ["--timezone", TZ[os_name]]
    if locale:
        cmd += ["--locale", locale]
...
def check(cfg, launch, rc, rep, log):
    bad = []
...
def main():
    ...
    for os_name in OSES:
        for i in range(N):
            ...
            label = f"{os_name}#{i}"
            bad = check(cfg, launch, rc, rep, log)
            results[label] = not bad
            if bad:
                notes.append(f"{label}: " + "; ".join(bad))
    # Z: no --timezone. The zone comes from settings/locale_zones.json for the
    # locale; the browser must report exactly the emitted zone, still with no
    # camoucfg: line under strict (the table's unit tests never open a browser).
    zones = json.loads((CLIENT / "settings" / "locale_zones.json").read_text(encoding="utf-8"))["zones"]
    for tag in ZONE_LOCALES:
        gen = generate("windows", seed=7, locale=tag, timezone=False)
        cfg, launch = gen["config"], gen["launch"]
        rc, rep, log = probe(url, cfg, launch)
        bad = check(cfg, launch, rc, rep, log)
        if cfg.get("timezone:id") not in zones.get(tag, []):
            bad.append(f"Z zone {cfg.get('timezone:id')!r} not in the table for {tag}")
        label = f"Z {tag} no --timezone: zone {cfg.get('timezone:id')} from the table, reported by the browser"
        results[label] = not bad
        if bad:
            notes.append(f"{label}: " + "; ".join(bad))
