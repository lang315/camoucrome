...
| `settings/` | `invariants.json` (cross-surface invariant registry, SP5), `keys.json` (key registry), `presets/` (captured device presets, SP5b), `launcher.json` (the client contract), `build-args.gn` (canonical GN args, incl. the proprietary-codec pair) and `release-args.gn` (the non-component release shape `package.py` expects) |
| `scripts/` | `apply.sh` (the applier), `verify_*.py` (per-slice browser verifications), `package.py` (release archive from GN's runtime-deps list; refuses a component build, an off-pin version, a missing dep, mismatched stamps) |
