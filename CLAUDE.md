...
| `settings/` | `invariants.json` (cross-surface invariant registry, SP5), `keys.json` (key registry), `presets/` (captured device presets, SP5b), `launcher.json` (the client contract), `build-args.gn` (canonical GN args, incl. the proprietary-codec pair) `release-args.gn` (the non-component release shape `package.py` expects), `fonts.json` (the OFL font bundle manifest + captured Windows/macOS family lists + generated alias maps), `fontconfig/` (generated per-OS confs), `webgl/` (captured GPU profiles) and `locale_zones.json` (locale → plausible IANA zones) |
| `scripts/` | `apply.sh` (the applier), `verify_*.py` (per-slice browser verifications), `package.py` (release archive from GN's runtime-deps list; refuses a component build, an off-pin version, a missing dep, mismatched stamps), `winhost.py` (stock Chrome on the box's Windows host over ssh: headless `--dump-dom`, headed CDP), `capture_*.py` (baselines, presets, WebGL profiles, font lists, the Windows host oracle), `verify_host_oracle.py` (the fork under a generated Windows identity vs stock Chrome on the host, every differing leaf as a line) |
