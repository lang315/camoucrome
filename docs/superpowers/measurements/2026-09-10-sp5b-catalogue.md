...
| `fonts-alias-local-requires-list` | `fonts:aliasLocal` → `fonts:list` | `requires-key` | the `src:local()` twin: a page could `local("Selawik-Regular")` the bundle's own face beside a `local("SegoeUI")` that lands on it (added 2026-09-11, measurements/2026-09-11-fonts-metrics.md §4) |
