...
| component updater, the shared choke | `ConfiguratorImpl::UpdateUrl()` returns `{}` unless `--component-updater=url-source` is set; `update_client` then fails every check with `ProtocolError::MISSING_URLS` (`update_checker.cc:171`) before a socket opens. Found by the first GREEN attempt: with registration removed, on-demand registrants (IWA key distribution, optimization guide, on-device translation — `iwa_key_distribution_component_installer.cc:47-51` even documents ignoring the switch in tests) still produced 3 `update2/json` checks and 1 CRX download | `components/component_updater/configurator_impl.cc` |
