...
| a presence-only oracle for a claim-gated interface | **that the interface answers.** windows-oracle exposed `navigator.share` under a Windows claim (renderer gate) and O1 matched stock leaf for leaf; the first `share()` with a gesture asked the broker for `blink.mojom.ShareService`, Linux has no binder, and `ReportNoBinderForInterface` killed the renderer ("Target crashed", 2026-09-12). **Exposing a renderer interface under a claim requires its browser-side binder on the build OS**; the oracle must call each newly exposed method once, not only enumerate it (`windows-behaviour`). |
