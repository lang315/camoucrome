...
- **`distortPoints` micro-tremor.** The camoufox reference superimposes a Gaussian y-tremor on the interior curve points; the port keeps the Bézier bow and timing jitter but ships smooth cubics. Deferred by user decision 2026-08-29: the core humanization (curve, seed-dependence, timing jitter) is test-and-mutant-proven, tremor's marginal anti-detect value is unmeasured, and a smooth Bézier is sufficient against basic bot-detection (sannysoft/CreepJS). If a threat model with behavioral-biometric detectors arises, tremor is a self-contained follow-up: a Box-Muller draw off the existing SplitMix64, applied to interior points, plus one test that interior points deviate from the pure Bézier. A perfect smooth cubic is itself a potential tell, so this is a real fidelity gap, chosen not a missed one.
